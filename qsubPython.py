...
def allresources():
    ...
    shell_run = subprocess.run(
        "qstat -u $USER | cut -c1-8 | grep -Eo '[0-9]{1,7}' | tr '\n' ' '",
        shell=True,
        capture_output=True,
        encoding="utf-8",
    )
    if shell_run.stderr:
        raise RuntimeError(shell_run.stderr)
    ids = shell_run.stdout.split()

    resources_dict = {}
    for id in ids:
        shell_run = subprocess.run(
            f"""qstat -f {id} | grep -e resources_used -e Job_Name -e Resource_List.mem -e Resource_List.ncpus""",
            shell=True,
            capture_output=True,
            encoding="utf-8",
        )
...
def resources_out_to_dict(shell_out):
    ...
    if "resources_used.mem" in resources_dict.keys():
        ...
        resources_dict["resources_used.mem"] = to_gb(
            resources_dict["resources_used.mem"]
        )
        resources_dict["resources_used.vmem"] = to_gb(
            resources_dict["resources_used.vmem"]
        )
    ...
    def set_job_attrs(self, **kwargs):
        ...
        for k in keys:
            if k not in class_keys:
                raise AttributeError(f"{k} is not an attr and cannot be set")
    ...
    def submit(self, commands: List[str] = []):
        ...
        if stderr:
            raise ChildProcessError(
                f"Error: {stderr}.\n Please see qsub script {temp_qsub_file}"
            )  # doesn't delete the qsub script
        elif not jobID:
            warnings.warn(
                f"No jobID returned, job may not have submitted. please check qstat. use makefile() method to diagnose problems"
            )
        else:
            print(jobID)
